...
bool setupStorage() {
  ...
  // Good to go
  Serial.printf("initialized successfully. Capacity: %u\r\n", MB85RC.getDeviceCapacity());
  return true;
}
...
void initFileReceiver(uint32_t checksum, uint16_t dataSize) {
  // Sanity checks
...
bool writeSector(byte *data, uint16_t dataSize, byte sector) {
  ...
  // Selected bit in submask (for the mask we start at 1, not 0)
